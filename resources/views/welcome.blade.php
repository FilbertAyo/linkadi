    ...
    <section id="pricing" class="bg-white dark:bg-gray-900">
        <div class="max-w-screen-xl px-4 py-8 mx-auto lg:py-24 lg:px-6">
            <div class="max-w-screen-md mx-auto mb-8 text-center lg:mb-12">
                ...
                <p class="mb-5 font-light text-gray-500 sm:text-xl dark:text-gray-400">Select the perfect NFC card package for your needs</p>
            </div>
            
            @if($packages->isEmpty())
                <div class="text-center py-12">
                    <p class="text-gray-500 dark:text-gray-400">No packages available at the moment. Please check back later.</p>
                </div>
            @else
                <div class="grid grid-cols-1 gap-8 md:grid-cols-2 lg:grid-cols-3">
                    @foreach($packages->flatten() as $package)
                        <div class="flex flex-col p-6 mx-auto max-w-lg text-center text-gray-900 bg-white border border-gray-200 rounded-lg shadow dark:border-gray-700 xl:p-8 dark:bg-gray-800 dark:text-white">
                            @if($package->image)
                            ...
                            @if($package->description)
                                <p class="font-light text-gray-500 sm:text-lg dark:text-gray-400 mb-4">{{ Str::limit($package->description, 100) }}</p>
                            @endif
                            ...
                            <a href="{{ route('packages.show', $package->slug) }}" class="text-white bg-brand-600 hover:bg-brand-700 focus:ring-4 focus:ring-brand-200 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-brand-600 dark:hover:bg-brand-600 dark:focus:ring-brand-800">
                                @auth
                                    Order Now
                                @else
                                    Get Started
