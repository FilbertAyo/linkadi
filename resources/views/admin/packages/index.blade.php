    ...
    <div class="pt-6">
        <div class="mb-6 flex justify-between items-center">
            <div>
                ...
                <p class="text-gray-600 mt-2">Manage all packages in the system</p>
            </div> <a href="{{ route('admin.packages.create') }}" class="px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700"> Add New Package </a>
        ...
        <div class="bg-white shadow rounded-lg p-4 mb-6 border border-gray-200">
            <form method="GET" action="{{ route('admin.packages.index') }}" class="flex gap-4">
                <div class="flex-1"> <input type="text" name="search" value="{{ request('search') }}" placeholder="Search by name or description..." class="w-full px-4 py-2 border border-gray-300 rounded-lg bg-white text-gray-900"> </div>
                <div class="w-48"> <select name="type" class="w-full px-4 py-2 border border-gray-300 rounded-lg bg-white text-gray-900">
                        <option value="">All Types</option>
                        <option value="nfc_card" {{ request('type') === 'nfc_card' ? 'selected' : '' }}>NFC Card</option>
                        <option value="classic" {{ request('type') === 'classic' ? 'selected' : '' }}>Classic</option>
                    </select> </div>
                <div class="w-48"> <select name="status" class="w-full px-4 py-2 border border-gray-300 rounded-lg bg-white text-gray-900">
                        <option value="">All Status</option>
                        <option value="active" {{ request('status') === 'active' ? 'selected' : '' }}>Active</option>
                        <option value="inactive" {{ request('status') === 'inactive' ? 'selected' : '' }}>Inactive</option>
                    </select> </div> <button type="submit" class="px-4 py-2 bg-gray-600 text-white rounded-lg hover:bg-gray-700">Filter</button> @if(request()->hasAny(['search', 'type', 'status'])) <a href="{{ route('admin.packages.index') }}" class="px-4 py-2 bg-gray-400 text-white rounded-lg hover:bg-gray-500">Clear</a> @endif
            </form>
        </div> <!-- Packages Table -->
