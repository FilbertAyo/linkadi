...
<head>
    ...
    <meta name="description" content="{{ $profile->bio ? \Illuminate\Support\Str::limit($profile->bio, 160) : 'View ' . $profile->user->name . '\'s digital profile on Linkadi.' }}">
    ...
    <meta property="og:description" content="{{ $profile->bio ? \Illuminate\Support\Str::limit($profile->bio, 200) : 'View ' . $profile->user->name . '\'s digital profile' }}">
    ...
    <meta property="twitter:description" content="{{ $profile->bio ? Str::limit($profile->bio, 200) : 'View ' . $profile->user->name . '\'s digital profile' }}">
    @if($profile->profile_image)
        ...
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 -mt-16 pb-12">
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-xl overflow-hidden">
                    ...
                    <div class="flex flex-col sm:flex-row items-start sm:items-center gap-6">
                        ...
                        @if($profile->profile_image)
                            <img src="{{ asset('storage/' . $profile->profile_image) }}" 
                                 ...
                                 class="h-32 w-32 rounded-full border-4 border-white dark:border-gray-800 shadow-lg object-cover">
                        @else
                            <div class="h-32 w-32 rounded-full border-4 border-white dark:border-gray-800 shadow-lg bg-indigo-100 dark:bg-indigo-900 flex items-center justify-center">
                ...
                <div class="px-6 sm:px-8 pb-6 border-t border-gray-200 dark:border-gray-700 pt-6">
                    <h2 class="text-xl font-semibold text-gray-900 dark:text-white mb-3">About</h2>
                    <p class="text-gray-700 dark:text-gray-300 whitespace-pre-wrap">{{ $profile->bio }}</p>
                </div>
                @endif
                ...
                <div class="px-6 sm:px-8 pb-6 border-t border-gray-200 dark:border-gray-700 pt-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <a href="{{ $profile->website }}" target="_blank" rel="noopener noreferrer" class="text-gray-700 dark:text-gray-300 hover:text-indigo-600 dark:hover:text-indigo-400">
                ...
                <div class="px-6 sm:px-8 pb-8 border-t border-gray-200 dark:border-gray-700 pt-6">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                        @foreach($profile->socialLinks as $link)
                            <a href="{{ $link->url }}" 
                               ...
                               class="flex items-center gap-3 p-4 bg-gray-50 dark:bg-gray-700 hover:bg-gray-100 dark:hover:bg-gray-600 rounded-lg transition-colors group">
                                <div class="flex-shrink-0">
                                    ...
                                    @elseif($link->platform === 'twitter')
                                        <svg class="w-6 h-6 text-blue-400 dark:text-blue-300" fill="currentColor" viewBox="0 0 24 24">
                                    ...
                                    @elseif($link->platform === 'github')
                                        <svg class="w-6 h-6 text-gray-800 dark:text-gray-200" fill="currentColor" viewBox="0 0 24 24">
                                ...
                                <span class="font-medium text-gray-900 dark:text-white group-hover:text-indigo-600 dark:group-hover:text-indigo-400">
                                    {{ $link->label }}
                                </span>
                            </a>
                        @endforeach
                ...
                <!-- Footer -->
                <div class="px-6 sm:px-8 py-4 bg-gray-50 dark:bg-gray-900 border-t border-gray-200 dark:border-gray-700">
                    <p class="text-sm text-center text-gray-500 dark:text-gray-400">
                        Powered by <a href="{{ route('welcome') }}" class="text-indigo-600 dark:text-indigo-400 hover:underline">Linkadi</a>
                    </p>
                </div>
            </div>
        </div>
